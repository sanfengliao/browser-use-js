...
export function highlightElement(args = {
    ...
    try {
      ...
      if (firstRect.width < labelWidth + 4 || firstRect.height < labelHeight + 4) {
        ...
        if (labelLeft < iframeOffset.x)
          labelLeft = firstRectLeft // Prevent going off-left
      }
  ...
  function handleHighlighting(nodeData, node, parentIframe, isParentHighlighted) {
    if (!nodeData.isInteractive)
      return false // Not interactive, definitely don't highlight
  ...
  function buildDomTree(node, parentIframe = null, isParentHighlighted = false) {
    ...
    if (!node || node.id === HIGHLIGHT_CONTAINER_ID
      ...
      if (debugMode)
        PERF_METRICS.nodeMetrics.skippedNodes++
      return null
    }
